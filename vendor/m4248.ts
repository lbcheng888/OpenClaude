// @ts-nocheck
import {gw,$A} from "../src/config/2711_WORKFLOW_TOOL_NAME.ts";
...
import {tI,Rm} from "./m465.ts";
import {Hn} from "./m100.ts";
async function Lja(){let e=gw(),t=Yc(),n=t?Mo:ws,r=await UUp(),o="`shQuote(s)` is POSIX-only \u2014 for PowerShell, double the single quotes: `\"'\"+s.replaceAll(\"'\", \"''\")+\"'\"`. For multi-line input use a here-string `@'\\n...\\n'@` (closing `'@` at column 0).",s=r?`gh pr edit N --body-file - <<'EOF'\\n"+body+"\\nEOF`:`git commit -F - <<'EOF'\\n"+msg+"\\nEOF`;if(e)return`
...
- \`await ${fa}({\u2026})\` / \`await ${Y0}({\u2026})\` / \`await mcp__server__tool({\u2026})\` (MCP tools by full name)
...
`}
function Mja(){return gw()?"Execute JavaScript to read, write, edit files and run shell commands":"Execute JavaScript code with access to Claude Code tools"}
var UUp;
var Nja=b(()=>{YU();Zm();tI();ry();$A();UUp=Hn(async()=>await Rm("gh")!==null)});
export {Lja,Mja,UUp,Nja};
