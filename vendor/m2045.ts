...
function YBr(e){return e.replace(/[&<>"']/g,(t)=>Itd[t]??t)}
function Pse(e){let{ok:t,heading:n,message:r,detail:o}=e,s=t?'<span class="status">Connected</span>':'<span class="status err">Error</span>',i=o?`<div class="detail">${YBr(o)}</div>`:"",a=t?"<script>setTimeout(function(){try{window.close()}catch(e){}},1500)</script>":"";return`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Claude Code</title><style>${xtd}</style></head><body><main>${s}<h1>${YBr(n)}</h1><p class="sub">${YBr(r)}</p>${i}</main>${a}</body></html>`}
var Itd,xtd=`*,*::before,*::after{box-sizing:border-box}
