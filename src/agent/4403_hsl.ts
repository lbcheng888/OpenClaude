...
import {mE as cD,Jm as Bz} from "../config/2207_Jm.ts";
import {vs as V9,dm as lT} from "../../vendor/m2256.ts";
import {readRoster as g1,XR as RP} from "../../vendor/m2707.ts";
import {su as B5,ow as qZ} from "../../vendor/m2257.ts";
import {Ec as t1,dw as mW} from "../../vendor/m2593.ts";
import {fa as xK,ry as dw} from "../../vendor/m2253.ts";
 ...
 */
function UdK(recentMessageCount: number, existingMemoryFilesText: string, includeScopeGuidance: boolean): string {
  let useUnixShell = $5(),
    fileToolName = useUnixShell ? aq : K7,
    readOnlyToolList = useUnixShell ? "ls/find/cat/stat/wc/head/tail and similar" : "Get-ChildItem/Get-Content/Select-Object -First/-Last and similar",
    deleteCommand = useUnixShell ? "rm" : "Remove-Item",
    immutableMemories = cD(),
    dedupeGuidance = immutableMemories ? `Check this list before writing — if the fact is already covered, skip it; if a memory has gone stale, ${deleteCommand} it and write a fresh single-fact memory in its place. Never edit memories in-place.` : "Check this list before writing — update an existing file rather than creating a duplicate.",
    existingFilesSection = existingMemoryFilesText.length > 0 ? `
...
${dedupeGuidance}` : "",
    ...
    availableToolsText = immutableMemories ? `Available tools: ${V9}, ${g1}, ${B5}, read-only ${fileToolName} (${readOnlyToolList}), ${t1} for paths inside the memory directory only, and ${fileToolName} ${deleteCommand} with paths inside the memory directory only. ${xK} is not permitted — memories are immutable, so delete-and-recreate replaces in-place edits. All other tools — MCP, Agent, write-capable ${fileToolName}, etc — will be denied.` : `Available tools: ${V9}, ${g1}, ${B5}, read-only ${fileToolName} (${readOnlyToolList}), and ${xK}/${t1} for paths inside the memory directory only, and ${fileToolName} ${deleteCommand} with paths inside the memory directory only. All other tools — MCP, Agent, write-capable ${fileToolName}, etc — will be denied.`,
    turnBudgetText = immutableMemories ? `You have a limited turn budget. Issue all ${t1} and ${deleteCommand} calls in parallel in a single turn — there is no read-then-edit dance, since memories are immutable.` : `You have a limited turn budget. ${xK} requires a prior ${V9} of the same file, so the efficient strategy is: turn 1 — issue all ${V9} calls in parallel for every file you might update; turn 2 — issue all ${t1}/${xK} calls in parallel. Do not interleave reads and writes across multiple turns.`;
  return [`You are now acting as the memory extraction subagent. Analyze the most recent ~${recentMessageCount} messages above and use them to update your persistent memory systems.`, "", availableToolsText, "", turnBudgetText, "", `You MUST only use content from the last ~${recentMessageCount} messages to update your persistent memories. Do not waste any turns attempting to investigate or verify that content further — no grepping source files, no reading code to confirm a pattern exists, no git commands.` + existingFilesSection, "", "If nothing is worth saving, output only 'Nothing to save.' Do not explain why.", "", "If the user explicitly asks you to remember something, save it immediately as whichever type fits best. If they ask you to forget something, find and remove the relevant entry.", "", `Apply the memory types, ${scopeGuidanceFragment}what-not-to-save criteria, and frontmatter format from the Memory section of your system prompt — it is already in your context above.`].join(`
`);
}
