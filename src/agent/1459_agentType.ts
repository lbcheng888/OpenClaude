...
/** 提取子 Agent 调用上下文标签（subagent_type + is_built_in_agent，异常回退空对象） */
function getSubAgentInvocationTags(ctx: AgentContext): Record<string, unknown> {
  try {
    if (!isSubAgent(ctx)) return {};
    let displayName = getSubAgentDisplayName(ctx);
    if (displayName === void 0) return {};
    return {
      subagent_type: displayName,
      is_built_in_agent: ctx.isBuiltIn ?? !1
    };
  } catch {
    return {};
  }
...
export {runWithAgentContext as Q5,createMainAgentContext as initProfileReportModule,isMainAgent as y7,getAgentDepth as k3,isSubAgent as NAe,getSubAgentDisplayName as _xr,getSubAgentInvocationTags as bXe,extractInvocationEvent as yxr,asyncHooksModule as M5s,agentContextStorage as Ukt,initModule as Ph};
