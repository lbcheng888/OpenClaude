// @ts-nocheck
import {OUa as k0K,X2n as Mv6} from "../telemetry/4054_mediaType.ts";
import {qZ as nt} from "../../vendor/m2510.ts";
...
declare function k0K(text: string): { totalLines: number; truncatedContent: string; isImage: unknown };
...
declare function Q_(): { readFileBytes(path: string): Promise<Buffer> };
...
interface ProcessedNotebookOutput {
  ...
  image?: { image_data: string; media_type: string };
}
 ...
 */
function extractImageOutput(data: Record<string, unknown>): { image_data: string; media_type: string } | undefined {
...
async function readNotebookCells(notebookPath: string, cellId?: string): Promise<ProcessedNotebookCell[]> {
  let resolvedPath = Z9(notebookPath),
    ...
    notebook: { cells: RawNotebookCell[]; metadata?: { language_info?: { name?: string } } };
  try {
    notebook = d_(rawText);
  } catch (error) {
...
var MAX_OUTPUT_CONTENT_LENGTH = 1e4,
  NotebookReadError: { new (message: string): Error };
...
export {outputsExceedSizeLimit as owp,truncateOutputText as Hlo,extractImageOutput as swp,convertNotebookOutput as iwp,convertNotebookCell as LUa,renderCellContent as awp,renderOutputContent as lwp,renderCell as cwp,readNotebookCells as MUa,buildNotebookToolResult as NUa,parseCellIndexFromId as B$t,MAX_OUTPUT_CONTENT_LENGTH as rwp,NotebookReadError as Ilo,Xv6 as Q2n};
