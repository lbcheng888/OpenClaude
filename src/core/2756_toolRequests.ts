// @ts-nocheck
import {kk as TZ,lo as zq} from "../tools/5190_userPromptCount.ts";
import {$f as Fz,HF as ZC} from "./2683_HF.ts";
...
function HM6(messages: Array<AttachmentEntry | unknown>): TokenUsageStats {
  ...
  return (
    (messages as AttachmentEntry[]).forEach((entry) => {
    ...
    (TZ(messages) as NormalizedMessage[]).forEach((message) => {
      ...
      if (typeof content === "string") {
        ...
        if (((stats.total += tokens), message.type === "user" && content.includes("local-command-stdout")))
          stats.localCommandOutputs += tokens;
        else stats[message.type === "user" ? "humanMessages" : "assistantMessages"] += tokens;
      } else
        content.forEach((block) =>
          XA3(block as ContentBlock, message, stats, toolNameById, filePathById, fileReadTallies)
        );
    }),
...
function XA3(
  ...
  fileReadTallies: Map<string, FileReadTally>
): void {
  ...
  switch (((stats.total += tokens), block.type)) {
    case "text":
      if (
        message.type === "user" &&
        "text" in block &&
        (block.text as string).includes("local-command-stdout")
      )
        stats.localCommandOutputs += tokens;
      else stats[message.type === "user" ? "humanMessages" : "assistantMessages"] += tokens;
      break;
    case "tool_use": {
      if ("name" in block && "id" in block) {
        ...
        if (
          (XN7(stats.toolRequests, toolName, tokens),
          toolNameById.set(block.id as string, toolName),
          toolName === "Read" &&
            "input" in block &&
            block.input &&
            typeof block.input === "object" &&
            "file_path" in (block.input as object))
        ) {
          let filePath = String((block.input as { file_path: unknown }).file_path);
...
function _M6(stats: TokenUsageStats): TokenUsageTelemetry {
  ...
  let duplicateReadTokens = [...stats.duplicateFileReads.values()].reduce(
    (sum, entry) => sum + entry.tokens,
    0
  );
  if (
    ((telemetry.duplicate_read_tokens = duplicateReadTokens),
    (telemetry.duplicate_read_file_count = stats.duplicateFileReads.size),
    stats.total > 0)
  ) {
    (telemetry.human_message_percent = Math.round((stats.humanMessages / stats.total) * 100)),
      (telemetry.assistant_message_percent = Math.round((stats.assistantMessages / stats.total) * 100)),
      (telemetry.local_command_output_percent = Math.round((stats.localCommandOutputs / stats.total) * 100)),
      (telemetry.duplicate_read_percent = Math.round((duplicateReadTokens / stats.total) * 100));
    ...
    (telemetry.tool_request_percent = Math.round((toolRequestTotal / stats.total) * 100)),
      (telemetry.tool_result_percent = Math.round((toolResultTotal / stats.total) * 100)),
      stats.toolRequests.forEach((tokens, toolName) => {
...
export {HM6 as dRn,XA3 as hxd,XN7 as v2i,_M6 as pRn,Op8 as h5r};
