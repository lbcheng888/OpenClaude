...
import {Xd as WO,Ct as L_} from "../../vendor/m197.ts";
import {Vi as KK,H1e as eNH,$d as ZO} from "../config/0620_$d.ts";
import {ho as $q} from "../../vendor/m572.ts";
import {getOauthConfig as F9,Sc as u1} from "../api/0465_getOauthConfig.ts";
import {getUserAgent as An,kk as g0} from "../api/2037_withOAuth401Retry.ts";
...
import {isAnthropicAuthEnabled as sf,getClaudeAIOAuthTokens as H7,checkAndRefreshOAuthTokenIfNeeded as sY,lo as Mq} from "../config/2036_withOAuthRefreshLock.ts";
import {logForDebugging as N,qe as FH} from "../config/0236_setHasFormattedOutput.ts";
import {getClientPlatform as L0} from "../../vendor/m5.ts";
import {woe as S8H,zK as El} from "../config/0751_bytes.ts";
import {getWebSocketProxyUrl as ks,ey as Af} from "../config/1026_shouldBypassProxyWithCidr.ts";
...
function formatCodeInRange(value: unknown, min: number, max: number): string {
  return typeof value === "number" && Number.isInteger(value) && value >= min && value <= max ? String(value) : "unknown";
}
 ...
 */
async function connectVoiceStream(handlers: VoiceStreamHandlers, options?: VoiceStreamOptions): Promise<VoiceStreamController | null> {
  await sY();
  let auth = H7();
...
export {p04 as FUl,formatCodeInRange as wJn,errorCodeForTelemetry as yCm,probeVoiceConnectivity,isTypedInterimsEnabled,isVoiceStreamAvailable,sanitizeKeytermsForHeader,connectVoiceStream,WsModule as aSe,KEEP_ALIVE_FRAME as LUl,CLOSE_STREAM_FRAME as fCm,noopConsoleError as hCm,VOICE_STREAM_PATH as gCm,KEEP_ALIVE_INTERVAL_MS as _Cm,FINALIZE_TIMEOUTS_MS,PROBE_TIMEOUT_MS as TCm,KEYTERMS_HEADER_MAX_LENGTH as SCm,aMq as UDo};
