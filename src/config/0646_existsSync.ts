...
import {v5 as jF,Lv as qW} from "../../vendor/m643.ts";
import {bje as Wj_,Dyr as u18} from "../../vendor/m644.ts";
/**
...
function Fs_(): void {
  if (t_() === "windows") {
    let gitBashPath = j3H();
    if (gitBashPath) process.env.SHELL = gitBashPath, N(`Using bash path: "${gitBashPath}"`);else N("Git Bash not found; BashTool will be unavailable");
  }
}
...
function Qs_(rawCommand: string): string {
  ...
  while (pos < trimmed.length) {
    ...
    } else if (ch === "\\" && pos + 1 < trimmed.length) parsed += trimmed[pos + 1], pos += 2;else if (/\s/.test(ch)) break;else parsed += ch, pos++;
  }
...
var DI = L(() => {
  ...
  j3H = V6(() => {
    ...
    if (process.env.CLAUDE_CODE_GIT_BASH_PATH) {
      if (checkExists(process.env.CLAUDE_CODE_GIT_BASH_PATH)) return process.env.CLAUDE_CODE_GIT_BASH_PATH;
    ...
    let wellKnownPaths = ["C:\\Program Files\\Git\\bin\\bash.exe", "C:\\Program Files (x86)\\Git\\bin\\bash.exe"];
    for (let candidate of wellKnownPaths) if (checkExists(candidate)) return candidate;
  ...
  }, (win32Path: string) => win32Path, 500), TVH = qW((unixPath: string) => {
...
export {Fs_ as Brn,gs_ as Urn,Qs_ as $rn,sjH as GEe,j3H as Fpe,Nh as Qx,TVH as M1e,DI as r2};
