...
import {normalizePatternsToPath as b8_,getFileReadIgnorePatterns as I8_,nA as o$} from "../permissions/5145_untypeDenyReasonForAskPropagation.ts";
import {st as q_} from "../../vendor/m5.ts";
import {Jlt as C8_,L2t as Ay_} from "../../vendor/m3914.ts";
import {tae as e9H,oee as we} from "./2670_cause.ts";
...
function splitGlobPattern(globPattern: string): GlobSplitResult {
  ...
  let staticPrefix = globPattern.slice(0, firstMetaMatch.index),
    lastSepIndex = Math.max(
      staticPrefix.lastIndexOf("/"),
      staticPrefix.lastIndexOf(Rr.sep),
    );
  if (lastSepIndex === -1)
    return {
      baseDir: "",
...
async function d8K(
  ...
  if (Rr.isAbsolute(pattern)) {
    let { baseDir: splitBase, relativePattern: splitPattern } =
  ...
  let absoluteFiles = rawResults.map((filePath) =>
      Rr.isAbsolute(filePath) ? filePath : Rr.join(searchDir, filePath),
    ),
...
export {splitGlobPattern as $Tp,d8K as lLa,Rr as BY,l8K as cLa};
