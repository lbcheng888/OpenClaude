...
import {Xde as EkH,WKe as qQH,zf as $w} from "../../vendor/m133.ts";
import {k6o as Wyq,H6o as Zyq,w6o as Pyq} from "../../vendor/m135.ts";
import {In as b6,Dre as rY_,Ct as L_} from "../../vendor/m197.ts";
...
import {TeamDeleteToolName as bH,tn as H6} from "./0230_encoding.ts";
import {kc as x1,aA as SX} from "../../vendor/m234.ts";
import {writeToStderr as EQH,LP as tk} from "../../vendor/m232.ts";
 ...
 */
async function maybeRotateDebugLog(filePath: string, bytesWritten: number, maxSize: number = t11): Promise<void> {
  if (lkH < 0) lkH = await qS.stat(filePath).then((stats: {
    size: number;
  }) => stats.size).catch(() => 0);else lkH += bytesWritten;
 ...
 */
async function appendToLogFile(shouldMkdir: boolean, dirPath: string, logPath: string, content: string): Promise<void> {
 ...
 */
function logForDebugging(message: string, {
  level: logLevel
}: {
  level?: string;
} = {
...
export {_Sq as gWo,getArgvBeforeDashDash as JXt,enableDebugLogging,resetDebugCaches,resolveDebugFilePath as mWo,passesDebugFilter as tHc,setHasFormattedOutput,getHasFormattedOutput,maybeRotateDebugLog,resetDebugLogRotationForTest,computeFallbackLogPath as hWo,appendToLogFile as oHc,swallowError as kcr,getOrCreateLogWriter as sHc,flushDebugLogs,logForDebugging,getDebugLogPath,logAntError,qS as xN,F5H as upe,xH8 as wcr,getMinDebugLogLevel,eEq as fWo,isDebugMode,getDebugFilter,isDebugToStdErr,getDebugFilePath,UH8 as Dcr,t11 as nHc,nkH as TMe,al_ as jXt,lkH as yMe,qA_ as Vbt,sl_ as YXt,dH8 as Ncr,_debugModuleInit as qe};
