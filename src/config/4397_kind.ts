// @ts-nocheck
import {K9 as R3,tQt as bYt,RX as xX,nQt as EYt} from "../../vendor/m241.ts";
...
function C4t() {
  if (!E4t) return;
  R3().clearMarks(), eho.clear(), ZAo = null, bQa++, am("query_user_input_received");
}
function am(e) {
  if (!E4t) return;
  let t = R3();
  if (t.mark(e), eho.set(e, process.memoryUsage()), e === "query_first_chunk_received" && ZAo === null) {
    let n = t.getEntriesByType("mark");
    if (n.length > 0) ZAo = n.at(-1)?.startTime ?? 0;
  }
}
function EQa() {
  if (!E4t) return;
  am("query_profile_end");
}
function e$p(e, t) {
...
function t$p() {
  if (!E4t) return "Query profiling not enabled (set CLAUDE_CODE_PROFILE_QUERY=1)";
  let t = R3().getEntriesByType("mark");
  if (t.length === 0) return "No query profiling checkpoints recorded";
  let n = [];
  n.push("=".repeat(80)), n.push(`QUERY PROFILING REPORT - Query #${bQa}`), n.push("=".repeat(80)), n.push("");
  ...
  for (let c of t) {
    let u = c.startTime - r,
      d = c.startTime - o;
    if (n.push(bYt(u, d, c.name, eho.get(c.name), 10, 9, e$p(d, c.name))), c.name === "query_api_request_sent") s = u;
  ...
  if (n.push(""), n.push("-".repeat(80)), i > 0) {
    let c = s,
      ...
      d = (c / i * 100).toFixed(1),
      p = (u / i * 100).toFixed(1);
    n.push(`Total TTFT: ${xX(i)}ms`), n.push(`  - Pre-request overhead: ${xX(c)}ms (${d}%)`), n.push(`  - Network latency: ${xX(u)}ms (${p}%)`);
  } else n.push(`Total time: ${xX(l)}ms`);
  return n.push(n$p(t, r)), n.push("=".repeat(80)), n.join(`
`);
}
function n$p(e, t) {
  let n = [{
  ...
  for (let i of n) {
    ...
    if (a !== void 0 && l !== void 0) {
      ...
      o.push(`  ${i.name.padEnd(22)} ${xX(c).padStart(10)}ms ${u}`);
    }
  }
  let s = r.get("query_api_request_sent");
  if (s !== void 0) o.push(""), o.push(`  ${"Total pre-API overhead".padEnd(22)} ${xX(s).padStart(10)}ms`);
  return o.join(`
`);
}
function Wqn() {
...
var B6e = b(() => {
  ...
  Lr();
  EYt();
  E4t = je.CLAUDE_CODE_PROFILE_QUERY, eho = new Map();
});
export {SQa as osl,C4t as J6t,am as initKp,EQa as isl,e$p as MWp,t$p as NWp,n$p as FWp,Wqn as cWn,E4t as Y6t,eho as VSo,bQa as ssl,ZAo as GSo,B6e as d8e};
