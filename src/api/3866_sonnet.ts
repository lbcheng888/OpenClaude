...
import {buildVertexGoogleAuth as toH,wyn as Iz6} from "../../vendor/m2024.ts";
import {nl as T1,T2 as bm} from "../../vendor/m1455.ts";
import {vyn as bz6,Ryn as Cz6} from "../core/2024_default.ts";
import {ey as Af,RNe as MnH} from "../config/1026_shouldBypassProxyWithCidr.ts";
import {KKe as TQH,dn as A6} from "../config/0137_namespace.ts";
 ...
 */
type VertexProbeResult = {
  ok: boolean;
  reason?: VertexProbeReason;
};
...
function listMatchingVertexModels(substring: string): string[] {
  ...
  for (let entry of Object.values(uz)) if (entry.vertex.toLowerCase().includes(substring)) matches.add(entry.vertex);
...
function resolveGoogleAuthCredential(config: VertexAuthConfig): GoogleAuthCredential {
  if (config.authMethod === "serviceAccount" && config.keyFile) return {
    kind: "keyFile",
    path: config.keyFile
  };
  return {
...
async function validateVertexCredentials(config: VertexAuthConfig): Promise<VertexCredentialCheck> {
  ...
  try {
    let googleAuth = await toH(resolveGoogleAuthCredential(config), config.projectId),
      ...
      timeoutPromise = new Promise((resolve, reject) => setTimeout(reject2 => reject2(Error("Timed out waiting for GCP credentials")), GCP_CREDENTIAL_TIMEOUT_MS, reject));
    ...
    identity = clientEmail ?? (config.authMethod === "serviceAccount" ? `service account (${config.keyFile})` : "Application Default Credentials");
  } catch (error) {
    return {
      status: "error",
...
async function createAnthropicVertexClient(config: VertexAuthConfig) {
  let [{
    ...
    }] = await Promise.all([Promise.resolve().then(() => (bz6(), Cz6)), Promise.resolve().then(() => (Af(), MnH))]),
 ...
 */
function describeVertexCredentialError(error: any, config: VertexAuthConfig): {
  error: string;
  command?: string;
} {
  let message = error?.message ?? String(error);
  if (config.authMethod === "serviceAccount" && /ENOENT|no such file/i.test(message)) return {
    error: `Service account key file not found: ${config.keyFile}`
  };
  if (/Could not load the default credentials/i.test(message)) return config.authMethod === "adc" ? {
  ...
  if (/invalid_grant|Token has been expired|reauth/i.test(message)) {
    if (config.authMethod === "serviceAccount") return {
    ...
    if (config.authMethod === "adc") return {
  ...
  if (/Unable to detect a Project Id/i.test(message)) return {
    error: "Could not determine a GCP project from the credentials. Go back and set the project ID explicitly."
  };
  if (/Timed out waiting for GCP/i.test(message)) return {
...
export {getVertexModelIds as oco,listMatchingVertexModels as eNa,resolveGoogleAuthCredential as tNa,validateVertexCredentials as nNa,probeVertexModelAuth as Sqe,createAnthropicVertexClient as ivp,describeVertexCredentialError as avp,GCP_CREDENTIAL_TIMEOUT_MS as svp,GCLOUD_ADC_LOGIN_COMMAND as rco,initModule as sco};
