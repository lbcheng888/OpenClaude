...
import {Sc as u1,ALLOWED_OAUTH_BASE_URLS as La_} from "./0465_getOauthConfig.ts";
/**
...
function cV4(urlString: string): string | null {
  ...
  if (approvedAnthropicHosts.has(parsedUrl.hostname)) {
    if (parsedUrl.protocol !== "wss:" && parsedUrl.protocol !== "https:") return `scheme ${jsonQuote(parsedUrl.protocol)} is not permitted for host ${jsonQuote(parsedUrl.hostname)}; only wss:// and https:// are accepted`;
    return null;
  }
...
var dV4 = L(() => {
  u1();
  approvedAnthropicHosts = new Set(["api.anthropic.com", "api-staging.anthropic.com", ...La_.map((baseUrl: string) => new URL(baseUrl).hostname)]);
});
export {cV4 as gGl,jsonQuote as TZn,approvedAnthropicHosts as wOm,dV4 as _Gl};
