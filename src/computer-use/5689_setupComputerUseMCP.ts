...
import {Q$e as KIH} from "../tools/3205_type.ts";
import {w_n as U16,_K as wn,wfe as LOH} from "./2193_iTerm_app.ts";
import {cot as $6_,uot as Y6_} from "../telemetry/3213_enabled.ts";
import {Y3 as Vl,scalar as Eh} from "../mcp/0728_serverName.ts";
import {_A as WY} from "../../vendor/m459.ts";
import {aot as T6_} from "../../vendor/m3206.ts";
var cs4 = {};
j_(cs4, {
 ...
 */
declare function KIH(
  capabilities: { screenshotFiltering: string; platform: string },
...
declare const U16: { screenshotFiltering: string; platform: string };
...
function setupComputerUseMCP(): {
  mcpConfig: Record<string, { type: string; command: string; args: string[]; scope: string }>;
  allowedTools: string[];
} {
  // Build the allowed-tools list: one qualified name per capability.
  let toolDefinitions = KIH(U16, $6_()).map(
      (toolDef: { name: string }) => Vl(wn, toolDef.name)
    ),
...
export {cs4 as qic,setupComputerUseMCP,pathModule as Uic,urlModule as $ic,ds4 as jic};
