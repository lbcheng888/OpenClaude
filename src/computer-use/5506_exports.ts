...
/**
 * Lazy CommonJS module whose `.exports` is the full source text of
 * `storybook/compare.mjs` — one of the bundled script assets shipped with the
 * Claude Code "design-sync" skill.
 *
 * This module contains NO executable comparison logic of its own. The factory
 * simply assigns a single template-literal STRING (the entire `compare.mjs`
 * program) to `module.exports`. At runtime that string is fetched by the skill
 * asset registry (see module 5470, key `"storybook/compare.mjs"`) and written
 * to disk / executed as a separate Node process. Everything between the
 * backticks below — including identifiers like `flag`, `hashDir`, `captureStory`,
 * `OUT`, etc. — is opaque payload text belonging to that other program, NOT
 * symbols of this module, and is preserved byte-for-byte.
 *
 * `compare.mjs` itself is a screenshot capture harness: it renders each
 * component's self-contained preview (`<Name>.html`) and the repo's own
 * Storybook reference side by side per story, writing the two true images and a
 * comparison sheet for a grading agent to judge. It deliberately computes no
 * pixel/similarity heuristics.
 *
 * `__commonJS` (the bundle preamble's `s` helper) wraps the factory in a
 * once-only lazy getter that returns `module.exports`.
 *
 * @returns the `compare.mjs` source as a string
 */
var LU4: () => string = s((_moduleExports: { exports: string }, module: { exports: string }) => {
