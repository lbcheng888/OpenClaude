// @ts-nocheck
import {Q as s} from "../../runtime.ts";
// NOTE on subsystem: despite the "computer-use" path, this module is NOT the
// macOS GUI-control code. It is a bundled asset module whose entire body is a
// single template-literal string: the verbatim source of the standalone Node
// script `package-capture.mjs`, part of Claude Code's "design-sync" tooling.
//
// `hU4` is the lazy CommonJS module wrapper for this asset. `s` is the bundle's
// shared esbuild module-init helper (cross-module; resolves at runtime):
...
//
// `package-capture.mjs` (the string payload) is the design-sync capture harness
// for the "PACKAGE" shape's absolute grading: it launches headless Chromium via
// Playwright, navigates each authored component preview's single-story render
// (`?story=`), screenshots every cell, and assembles per-component review sheets
// that an agent grades on an absolute rubric. Grades are keyed off the build-
// stamped `sourceKey` so styling/pipeline churn never invalidates a verdict;
// all state is campaign-local and gitignored under `.design-sync/.cache/review/`.
//
// IMPORTANT: every identifier inside the backtick string below (`flag`, `argv`,
// `manifest`, `comps`, etc.) is part of the emitted script's SOURCE TEXT, not
// this module's scope. They must NOT be renamed — doing so would alter the
// generated `package-capture.mjs`. The only renamable internal symbols here are
// the factory parameters.
//
// @returns the `package-capture.mjs` script source as a string (via module.exports).
var hU4 = s((moduleExports: Record<string, unknown>, moduleObject: {
  exports: string;
}) => {
  moduleObject.exports = `#!/usr/bin/env node
