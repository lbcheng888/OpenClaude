// @ts-nocheck
import {ARTIFACT_TOOL_NAME as FzH,uuidSlugFromUrl as Xe,sanitizeArtifactTitle as BD6,iee as gzH} from "./2713_uuidSlugFromUrl.ts";
import {nb as QJ} from "../config/2679_eee.ts";
...
function ur6(transcript: TranscriptEntry[]): ArtifactTranscriptState {
  ...
  for (let entry of transcript) {
    ...
    for (let block of entry.message.content as ToolUseBlock[]) if (block.type === "tool_use") {
      if (block.name === FzH) artifactToolUseIds.add(block.id);else if (block.name === QJ) webFetchToolUseIds.add(block.id);
    }
  }
  ...
  for (let entry of transcript) {
    ...
    for (let block of entry.message.content as ToolResultBlock[]) {
      ...
      if (artifactToolUseIds.has(block.tool_use_id)) applyArtifactResult(entry.toolUseResult, entry.timestamp, frameUrls, artifactReadVersions);else if (webFetchToolUseIds.has(block.tool_use_id)) applyWebFetchArtifactRead(entry.toolUseResult, artifactReadVersions);
    }
  }
  return {
    frameUrls,
 ...
 * that slug.
 */
function applyArtifactResult(toolUseResult: unknown, timestamp: string | undefined, frameUrls: FrameUrlMap, publishedVersions: ArtifactVersionMap): void {
  ...
  for (let [path, entry] of Object.entries(frameUrls)) if (path !== result.path && Xe(entry.url) === slug) delete frameUrls[path];
  delete frameUrls[result.path];
  let title = typeof result.title === "string" ? BD6(result.title) : null;
  if (frameUrls[result.path] = {
    url: result.url,
    updatedAt: Date.parse(timestamp as string) || 0,
    ...(title !== null && {
      title
    })
  }, typeof result.version === "string") publishedVersions[slug] = result.version;
}
...
function applyWebFetchArtifactRead(toolUseResult: unknown, readVersions: ArtifactVersionMap): void {
  ...
  if (!artifactRead || typeof artifactRead.slug !== "string" || Xe(`https://claude.ai/code/artifact/${artifactRead.slug}`) !== artifactRead.slug || typeof artifactRead.ver !== "string") return;
...
export {ur6 as Xnr,applyArtifactResult as pVm,applyWebFetchArtifactRead as mVm,dRq as v2o};
