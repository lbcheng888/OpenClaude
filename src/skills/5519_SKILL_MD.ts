...
/**
 * Bundled skill assets for the "Cowork Plugin Authoring" skill.
 *
 * This module is a generated asset bundle: each markdown document that ships
 * with the skill is embedded as a module-level string literal, and the public
 * exports expose the main SKILL.md body plus a map of its `references/*.md`
 * companion files. The module uses the lazy CommonJS-interop init pattern
 * (`b(() => { ... })`) so the export bindings are only materialized on first
 * access.
 *
 * Exports:
 *   SKILL_MD    - the full SKILL.md body (main skill instructions)
 *   SKILL_FILES - map of reference file path -> embedded markdown contents
 */

/** Embedded contents of `references/component-schemas.md`. */
var componentSchemasMd = `# Component Schemas
...
`;
/** Lazy-init placeholder for the `references/component-schemas.md` chunk. */
var initComponentSchemasMd = () => {};
/** Embedded contents of `references/example-plugins.md`. */
var examplePluginsMd = `# Example Plugins
...
`;
/** Lazy-init placeholder for the `references/search-strategies.md` chunk. */
var initSearchStrategiesMd = () => {};
/** Embedded contents of the main SKILL.md body for "Cowork Plugin Authoring". */
var skillMdBody = `# Cowork Plugin Authoring
...
`;
/** Lazy-init placeholder for the main SKILL.md body chunk. */
var initSkillMdBody = () => {};

/** Export namespace object populated by the lazy module initializer below. */
var skillModuleExports = {};
ft(skillModuleExports, {
  SKILL_MD: () => SKILL_MD,
  SKILL_FILES: () => SKILL_FILES
});

/** The full SKILL.md body for the "Cowork Plugin Authoring" skill. */
var SKILL_MD: string;
/** Map of `references/*.md` path -> embedded markdown contents. */
var SKILL_FILES: Record<string, string>;

/**
 * Lazy module initializer: wires the embedded string chunks into the public
 * `SKILL_MD` / `SKILL_FILES` exports on first access.
 */
var initSkillModule = b(() => {
  initComponentSchemasMd();
  initExamplePluginsMd();
  initMcpServersMd();
  initSearchStrategiesMd();
  initSkillMdBody();
  SKILL_MD = skillMdBody, SKILL_FILES = {
    "references/component-schemas.md": componentSchemasMd,
    "references/example-plugins.md": examplePluginsMd,
    "references/mcp-servers.md": mcpServersMd,
    "references/search-strategies.md": searchStrategiesMd
  };
});

export {componentSchemasMd as ytc,initComponentSchemasMd as _tc,examplePluginsMd as Stc,initExamplePluginsMd as Ttc,mcpServersMd as Etc,initMcpServersMd as btc,searchStrategiesMd as Atc,initSearchStrategiesMd as Ctc,skillMdBody as vtc,initSkillMdBody as Rtc,skillModuleExports as wtc,SKILL_MD as o4m,SKILL_FILES,initSkillModule as ktc};
