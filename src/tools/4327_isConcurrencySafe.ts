// @ts-nocheck
import {kG as fc,i6t as Ax_,Cmt as L1_} from "./4326_code.ts";
import {l6t as fx_,xl as c4} from "../../vendor/m4427.ts";
import {rl as u4,ri as M7} from "./2235_userFacingName.ts";
import {X3n as Rb6,z6e as qpH} from "./4092_done.ts";
 ...
 */
declare function fc(event: unknown): event is {
  type: string;
  [key: string]: unknown;
};
 ...
 */
declare function u4(tools: ToolDefinition[], name: string, toolAliases?: Record<string, string>): ToolDefinition | undefined;
 ...
 * as `Ax_`.
 */
declare function Ax_(block: ToolUseBlock, assistantMessage: AssistantMessageEnvelope | undefined, canUseTool: CanUseTool, context: ToolUseContext, getNow: () => string): AsyncGenerator<ToolExecEvent>;
 ...
 * as `Rb6`.
 */
declare function Rb6<T>(generators: Iterable<AsyncGenerator<T>>, concurrency?: number): AsyncGenerator<T>;
 ...
 */
async function* runToolUseBlocks(toolUseBlocks: ToolUseBlock[], assistantMessages: AssistantMessageEnvelope[], canUseTool: CanUseTool, toolUseContext: ToolUseContext): AsyncGenerator<ToolExecEvent> {
  let context = toolUseContext;
  for (let {
    isConcurrencySafe,
    blocks
  } of groupBlocksByConcurrencySafety(toolUseBlocks, context)) if (isConcurrencySafe) {
  ...
  } else for await (let event of runBlocksSerially(blocks, assistantMessages, canUseTool, context)) {
    if (fc(event)) {
      yield event;
      continue;
    }
 ...
 */
function groupBlocksByConcurrencySafety(toolUseBlocks: ToolUseBlock[], context: ToolUseContext): ConcurrencyGroup[] {
  return toolUseBlocks.reduce<ConcurrencyGroup[]>((groups, block) => {
    ...
    if (isConcurrencySafe && groups.at(-1)?.isConcurrencySafe) groups.at(-1)!.blocks.push(block);else groups.push({
      isConcurrencySafe,
      blocks: [block]
    });
    return groups;
 ...
 */
async function* runBlocksSerially(blocks: ToolUseBlock[], assistantMessages: AssistantMessageEnvelope[], canUseTool: CanUseTool, toolUseContext: ToolUseContext): AsyncGenerator<ToolExecEvent> {
  let context = toolUseContext;
  for (let block of blocks) {
    for await (let event of Ax_(block, assistantMessages.find(msg => msg.message.content.some(c => c.type === "tool_use" && c.id === block.id)), canUseTool, context, () => new Date().toISOString())) {
      if (fc(event)) {
        yield event;
        continue;
      }
 ...
 */
async function* runBlocksConcurrently(blocks: ToolUseBlock[], assistantMessages: AssistantMessageEnvelope[], canUseTool: CanUseTool, toolUseContext: ToolUseContext): AsyncGenerator<ToolExecEvent> {
  yield* Rb6(blocks.map(async function* (block) {
    yield* Ax_(block, assistantMessages.find(msg => msg.message.content.some(c => c.type === "tool_use" && c.id === block.id)), canUseTool, toolUseContext, () => new Date().toISOString()), yield {
      type: "set_in_progress_tool_use_ids",
      op: {
        action: "remove",
        ids: [block.id]
...
export {getMaxToolUseConcurrency as I3p,runToolUseBlocks as $yo,groupBlocksByConcurrencySafety as x3p,runBlocksSerially as D3p,runBlocksConcurrently as P3p,initToolExecutionModule as iel};
