 ...
 */
function KIH(capabilities: ExecutorCapabilities, coordinateMode: string, availableApps?: string[]): ToolDescriptor[] {
  let coordinateDescriptions: CoordinateAxisDescriptions = Vv3[coordinateMode],
    availableAppsSuffix = availableApps && availableApps.length > 0 ? ` Available applications on this machine: ${availableApps.join(", ")}.` : "",
    ...
    screenshotDescription = capabilities.screenshotFiltering === "native" ? "Take a screenshot of the primary display. Applications not in the session allowlist are excluded at the compositor level — only granted apps and the desktop are visible." : "Take a screenshot of the primary display. On this platform, screenshots are NOT filtered — all open windows are visible. Input actions targeting apps not in the session allowlist are rejected.";
  return [{
    name: "request_access",
 ...
 */
function yv3(coordinateDescriptions: CoordinateAxisDescriptions, availableAppsSuffix: string): ToolDescriptor[] {
...
var Vv3: CoordinateModeDescriptions, /** Shared JSON-Schema for a single batch/teach action; initialized by `$c8`. */
  $o7: JsonSchemaNode;
...
export {KIH as i3e,yv3 as Ijd,Vv3 as Hjd,$o7 as Rca,$c8 as DQr};
