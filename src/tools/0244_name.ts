...
import {WAIT_MAX_DURATION_S as jA_} from "../permissions/0243_deviceId.ts";
// Browser-automation tool definitions for the Claude Code browser/Chrome-extension subsystem.
//
// This module declares the catalog of browser tools (JavaScript execution, page reading,
// element finding, mouse/keyboard control, navigation, tab management, GIF recording,
// multi-browser connection, etc.) together with their JSON input schemas, plus a few
// helper constants/sets and a prompt-builder for the multi-browser selection flow.
//
// The module body is wrapped in the bundler's lazy module initializer `L(() => { ... })`
// (esbuild `__esm`), so the exported `var`s are forward-declared at top level and populated
// the first time the initializer runs.
//
// External (cross-module) symbols referenced here and intentionally left unrenamed:
//   - `L`   : the bundler's lazy module-init helper (esbuild `__esm`).
//   - `jA_` : the maximum number of seconds the `wait` action may sleep (value 10),
//             defined in the permissions/device-connection module (`permissions/0240_deviceId.ts`).
//   These are owned by other modules; renaming them here would break resolution, so they
//   are preserved verbatim.
declare const L: <T>(init: () => T) => () => T;
declare const jA_: number;

/**
 * JSON-schema property entry describing a single field of a browser tool's input.
 * Mirrors the JSON Schema subset used by these tool definitions.
 */
interface ToolInputSchemaProperty {
  ...
  enum?: string[];
  /**
   * Item schema for array-typed properties. Either a primitive item (e.g. `{ type: "number" }`)
   * or a full object schema (as used by `browser_batch.actions`).
   */
  items?: {
    type: string;
  ...
  minimum?: number;
  maximum?: number;
  /** Present for object-typed properties (e.g. the gif_creator `options` object). */
  properties?: Record<string, ToolInputSchemaProperty>;
}

/** JSON Schema (object form) for a browser tool's `input`. */
...
/** A single browser-automation tool definition exposed to the model. */
interface BrowserToolDefinition {
  name: string;
  /** Optional human-facing title (only set on a few tools, e.g. the tab-management tools). */
  title?: string;
  description: string;
...
/**
 * Build the system-prompt instruction that forces the model to ask the user which connected
 * browser to use before performing any browser action.
 *
 * @param askUserToolName - Name of the available "ask the user a question" tool, if one exists.
 *   When provided, the model is told to call that tool; otherwise it falls back to the generic
 *   "your ask-user tool (if available)" phrasing.
 */
function buildBrowserSelectionPrompt(askUserToolName: string | undefined): string {
  return `Before any browser action, you MUST call ${askUserToolName ? `the ${askUserToolName} tool` : "your ask-user tool (if available)"} with a question listing EVERY connected browser as a separate option (use the display name as the label, and include the deviceId in parentheses), plus one final option labeled exactly: "${SWITCH_BROWSER_OPTION_LABEL}" Do not skip any connected browser and do not pick one yourself. If the user picks a specific browser, call select_browser with that browser's deviceId. ` + "If the user picks the final option, call switch_browser — this sends a confirmation prompt to every connected Chrome extension and waits for the user to click Connect in the one they want; it also lets them name that browser.";
}

/** Tool name for the in-page JavaScript execution tool. */
var JAVASCRIPT_TOOL_NAME = "javascript_tool",
  /**
   * Set of browser tool names that are session/window/tab-management or shortcut tools
   * (as opposed to page-content interaction tools like navigate/computer/find/read_page).
   * Populated by the lazy module initializer below.
   */
  // FIXME: unverified predicate — membership is known but the exact role of this set
  // (e.g. "tools exempt from domain permission checks") is not confirmed from this module alone.
  MANAGEMENT_TOOL_NAMES: Set<string>,
  /**
   * Set of browser tool names that manage the connection to / selection of a specific browser
   * (switch_browser, list_connected_browsers, select_browser).
   * Populated by the lazy module initializer below.
   */
  BROWSER_SELECTION_TOOL_NAMES: Set<string>,
  /** Label for the final "let me pick the browser inside Chrome" option shown to the user. */
  SWITCH_BROWSER_OPTION_LABEL = "Open a confirmation screen in every connected Chrome extension and let me select the right one there.",
  /** The catalog of all browser-automation tool definitions. Populated by the initializer below. */
  BROWSER_TOOL_DEFINITIONS: BrowserToolDefinition[];

/**
 * Lazy module initializer. On first invocation it populates `MANAGEMENT_TOOL_NAMES`,
 * `BROWSER_SELECTION_TOOL_NAMES`, and `BROWSER_TOOL_DEFINITIONS`.
 */
var initBrowserTools = L(() => {
  MANAGEMENT_TOOL_NAMES = new Set(["tabs_context_mcp", "tabs_create_mcp", "tabs_close_mcp", "shortcuts_list", "shortcuts_execute", "resize_window", "switch_browser", "list_connected_browsers", "gif_creator"]), BROWSER_SELECTION_TOOL_NAMES = new Set(["switch_browser", "list_connected_browsers", "select_browser"]);
  BROWSER_TOOL_DEFINITIONS = [{
    name: JAVASCRIPT_TOOL_NAME,
    ...
    inputSchema: {
      ...
      properties: {
        ...
        duration: {
          ...
          minimum: 0,
          maximum: jA_,
          description: `The number of seconds to wait. Required for \`wait\`. Maximum ${jA_} seconds.`
        },
        scroll_direction: {
          type: "string",
...
export {buildBrowserSelectionPrompt as Hyt,JAVASCRIPT_TOOL_NAME as gyc,MANAGEMENT_TOOL_NAMES as kyt,BROWSER_SELECTION_TOOL_NAMES as Y3o,SWITCH_BROWSER_OPTION_LABEL as _yc,BROWSER_TOOL_DEFINITIONS as BROWSER_TOOLS,initBrowserTools as RGe};
