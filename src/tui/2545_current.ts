...
import {Z$r as GS8,gwe as z2H} from "../../vendor/m2466.ts";
import {logEvent as c,Ct as y_} from "../../vendor/m131.ts";
import {fromEnum as tH} from "../../vendor/m5.ts";
import {JK as ji} from "../../vendor/m2461.ts";
...
function useKeybindingChord(action: string, context: string, fallback: string): string {
  ...
  if (
    cf6.useEffect(
      () => {
        if (isChordMissing && !hasLoggedFallback.current)
      ...
      [isChordMissing, action, context, fallback, reason],
    ),
    chord === void 0
  )
    return fallback;
...
export {useKeybindingChord as qH,cf6 as EEn,gSH as Iwe};
