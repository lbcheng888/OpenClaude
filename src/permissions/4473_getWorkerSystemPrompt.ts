...
- Do not spawn subagents (${Cs} tool)
...
/** Returns the list of built-in coordinator agents (just WORKER_AGENT). */
function getCoordinatorAgents() {
  return [WORKER_AGENT];
}
var WORKER_AGENT;
/** Lazy initializer: populates WORKER_AGENT with its full descriptor. */
var grl = b(() => {
  Ph();
  WORKER_AGENT = {
    agentType: "worker",
...
export {hrl as nul,getWorkerSystemPrompt,getCoordinatorAgents,WORKER_AGENT,grl as rul};
