// @ts-nocheck
import {getRuntimeMainLoopModel as nR,parseUserSpecifiedModel as D9,getCanonicalName as _9,isOpus1mMergeEnabled as bM,isFableAvailable as _qH,getDefaultFableModel as $OH,getDefaultSonnetModel as MG,getDefaultOpusModel as yJ,getDefaultHaikuModel as mDH,Ro as iq} from "./1458_swapShrinksContextWindow.ts";
import {nl as T1,T2 as bm,kme as DyH} from "../../vendor/m1455.ts";
import {Oa as N4,eO as _v} from "../../vendor/m1456.ts";
import {Ldn as u96,qJe as anH,$oe as HOH} from "../config/1285_BedrockClient.ts";
import {getProviderForModel as jw,usesFirstPartyModelIds as OO,getAPIProvider as l8,Ps as V7} from "../api/1287_usesFirstPartyModelIds.ts";
import {logForDebugging as N,qe as FH} from "../config/0236_setHasFormattedOutput.ts";
import {X5 as bF,k_ as Uj,GS as xM} from "../api/2028_used.ts";
import {fk as v0,lr as P8} from "../../vendor/m233.ts";
 ...
 */
function resolveSubagentModel(agentModelConfig: string | undefined, mainLoopModel: string, explicitModelOverride: string | undefined, permissionMode: string | undefined, onFallback?: (requestedModel: string, resolvedModel: string) => void): string {
  let resolveInheritedModel = (): string => nR({
    ...
    warnAndInherit = (requestedModel: string, resolvedAlias: string = requestedModel): string => {
      ...
      if (T1(D9(resolvedAlias)).toLowerCase() !== T1(D9(inheritedModel)).toLowerCase()) onFallback?.(requestedModel, inheritedModel);
...
function buildSubagentModelOptions(): Array<{
  value: string;
  label: string;
  description: string;
}> {
  ...
  if ((_qH() || !OO() || l8() === "anthropicAws") && N4($OH())) options.push({
    value: "fable",
    label: "Fable",
    description: "Most capable for your hardest and longest-running tasks"
  });
  if (N4(MG())) options.push({
    value: "sonnet",
    label: "Sonnet",
    description: "Efficient for routine tasks"
  });
  if (N4(yJ())) options.push({
    value: "opus",
    label: "Opus",
    description: "Best for everyday, complex tasks"
  });
  if (N4(mDH())) options.push({
    value: "haiku",
    label: "Haiku",
...
export {defaultSubagentModelMode as Gco,resolveSubagentModel as pte,warnSubagentModelNotAllowed as Iwp,appendOneMillionContextSuffix as GFa,aliasMatchesModel as VFa,formatSubagentModelLabel as c$n,buildSubagentModelOptions as KFa,subagentModelValues as lK_,initSubagentModelModule as tce};
