...
import {ps as H9,formatFileSize as f4} from "../../vendor/m238.ts";
...
function isBase64ImageBlock(block: unknown): block is { source: { type: "base64"; data: string }; type: "image" } {
...
function isToolResultBlock(block: unknown): block is { type: "tool_result"; content: unknown[] } {
  if (typeof block !== "object" || block === null) return !1;
 ...
 */
function validateImageBase64Size(
  imageBlock: { source: { type: "base64"; data: string } },
  imageIndex: number,
  maxBytes: number,
  violations: Array<{ index: number; size: number }>
): void {
...
function Bx_(messages: unknown[], maxBytes: number): void {
  let violations: Array<{ index: number; size: number }> = [],
    imageCounter = 0;
...
var l1_: {
  new(violations: Array<{ index: number; size: number }>, maxBytes: number): Error & { name: string };
};
var ax6 = L(() => {
  y_();
  H9();
  l1_ = class l1_ extends Error {
    constructor(violations: Array<{ index: number; size: number }>, maxBytes: number) {
      let message: string,
        first = violations[0];
...
export {isBase64ImageBlock as hUi,isToolResultBlock as ARd,validateImageBase64Size as gUi,Bx_ as hOt,l1_ as snt,ax6 as Fwn};
