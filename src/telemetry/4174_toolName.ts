...
import {ws as K7} from "../config/2709_Zm.ts";
import {isDangerousBashPermission as oC_,isDangerousPowerShellPermission as sC_,fqn as gE6} from "../../vendor/m4172.ts";
import {Jf as d$,gA as $W} from "../mcp/0733_serverName.ts";
 ...
 */
function classifyShellAllowRule(toolName: string, ruleContent: string | undefined): "bare" | "dangerous_prefix" | "scoped" | null {
 ...
 */
function buildShellAllowRuleCounts(permissionsSettings: Record<string, Array<unknown> | undefined>): Record<string, number> {
  let counts: Record<string, number> = {},
    total = 0;
  for (let source of settingsSourceKeys) for (let entry of permissionsSettings[source] ?? []) {
    let {
        toolName: toolName,
 ...
 * that is being added.
 */
function logShellAllowRuleAdded(operations: Array<{
  type: string;
  behavior: string;
  rules: Array<{
    toolName: string;
...
export {normalizeShellToolName as Nho,classifyShellAllowRule as qVa,buildShellAllowRuleCounts as PNp,logShellAllowRulesAtInit as WVa,logShellAllowRuleAdded as hqn,settingsSourceKeys as DNp,cE6 as gqn};
