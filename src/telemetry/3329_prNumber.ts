...
import {getCommitCounter as Fs6,setSessionPrResolved as pt6,getPrCounter as XkH,lt as w_,$be as ifH} from "../session/0132_sent.ts";
import {D3e as vIH,rat as _8_} from "./3328_stdout.ts";
import {_a as iK,P3e as EIH} from "../permissions/5175_writeRemoteAgentMetadata.ts";
 ...
 */
function parsePrUrlInfo(text: string): {
  prNumber: number;
  prUrl: string;
  prRepository: string;
} | null {
 ...
 */
function parseLastPrUrl(text: string): {
  prNumber: number;
  prUrl: string;
  prRepository: string;
} | null {
 ...
 */
function parseGitOperationPayload(commandLine: string, stdoutOutput: string): {
  commit?: {
    sha: string;
  ...
  if (prActionEntry === "merged") {
    if (/--disable-auto\b/.test(commandLine)) prActionEntry = "auto-merge-disabled";else if (/--auto\b/.test(commandLine)) prActionEntry = "auto-merge-enabled";
 ...
 */
function observeCompletedCommand(commandLine: string, exitCode: number, stdoutOutput: string | undefined): void {
  ...
  if (checkoutMatch?.[1]) fetchAndLinkPr(checkoutMatch[1]).catch(() => {});else if (pushRegex.test(commandLine) && !prActionEntry) fetchAndLinkPr().catch(() => {});
...
var O8_ = L(() => {
  ...
  _8_();
  H6();
  commitRegex = buildGitCommandRegex("commit"), pushRegex = buildGitCommandRegex("push"), cherryPickRegex = buildGitCommandRegex("cherry-pick"), mergeRegex = buildGitCommandRegex("merge", "(?!-)"), rebaseRegex = buildGitCommandRegex("rebase"), gitOperationSchema = kH(() => k.object({
    commit: k.object({
      sha: k.string(),
  ...
  })), prCheckoutRegex = /\bgh\s+pr\s+checkout\b[^&|;]*\s(\d+)(?=\s|$|[&|;])/, ghPrActionTable = [{
  ...
  ghCommandRegex = /(?:^|[;&|]|\b(?:then|do)\b)\s*gh\s+(?!auth\b|help\b|version\b|alias\b|completion\b|config\b)/, rateLimitErrorRegex = /API rate limit (?:already )?exceeded|exceeded a secondary rate limit|\bRATE_LIMITED\b/i;
});
export {buildGitCommandRegex as oat,parsePrUrlInfo as xOn,parseLastPrUrl as vOn,extractCommitSha as _to,extractPushedBranch as uep,extractPrNumberFromOutput as dep,extractGitSubcommandArg as Gha,parseGitOperationPayload as sat,observeCompletedCommand as DOn,linkSessionToPrInfo as wOn,maybeRateLimitReminder as Yha,fetchAndLinkPr as Kha,commitRegex as zha,pushRegex as kOn,cherryPickRegex as iep,mergeRegex as aep,rebaseRegex as lep,gitOperationSchema as HOn,prCheckoutRegex as cep,ghPrActionTable as jha,prUrlRegex as IOn,ghCommandRegex as pep,rateLimitErrorRegex as mep,rateLimitReminderCooldownMs as fep,rateLimitReminderNextAt as Vha,O8_ as iat};
