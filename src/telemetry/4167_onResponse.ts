 ...
 */
function filterConnectedWithChannel(
  clients: McpClientEntry[],
  nameFilter: (name: string) => boolean
): McpClientEntry[] {
  return clients.filter(
    (client: McpClientEntry) =>
      client.type === "connected" &&
      nameFilter(client.name) &&
      client.capabilities?.experimental?.["claude/channel"] !== void 0 &&
      client.capabilities?.experimental?.["claude/channel/permission"] !== void 0
  );
}
...
export {isHarborPermissionsEnabled as zqa,hashWordRaw as Kqa,hashWord as Yqa,serializeShort as Jqa,filterConnectedWithChannel as Xqa,createChannelPermissionCallbacks as Qqa,SAFE_ALPHABET as jIp,BLOCKED_WORDS as WIp,x8q as edo};
